...
- 已知风险：dewarp 能力有限；部分样本（如 image_022 右缘）仍存在边缘漏检/带背景，需要继续优化 A4 边缘定位。
...
## 变更记录
- 2025-12-16：mask 主体筛选调整：仅当次大区域满足“标题长条且贴顶、与主体宽度重叠、占高<=40%”时才合并，其他细长小块一律不并入主体，避免把背页/翻折条纹混入裁剪范围。重新跑 image_022（outputs_fix022）确认右缘背景问题缓解但仍需后续边缘削减优化。
- 2025-12-16：分割小块合并规则收紧：仅在面积足够小且与主体距离占长边≤0.12时才合并，否则直接丢弃，防止远端小块（如 022 顶部翻页）拼入主体。image_022 复跑（outputs_fix022_gap2）顶部小块仍存在，说明需进一步处理贴边小块/贴边弱梯度削减。
- 2025-12-16：mask 清理策略迭代：改为“只保留最大连通域，并对顶部窄颈突出做切除”以丢弃翻页小块（segment `_keep_only_main_region` + `_strip_top_protrusion`），并用 connectedComponents 避免轮廓异常。image_022 复跑（outputs_fix022_topstrip）顶部突出已明显收缩，但仍有少量翻页残留，后续需进一步调节窄颈阈值或结合梯度削边。
- 2025-12-16：分割前预处理默认关闭（可选弱化参数），避免增强导致翻页并入；022/018 翻页问题在弱化/关闭后已消失。重构 segment 清理逻辑，新增 `segment.clean` 配置，统一“保留主连通域 + 上下贴边小块裁剪”，精简重复函数；回归样本输出在 `outputs_sample20_seed42_topcut/`。
...
- 2025-12-17：新增分割前预处理副本（对比度拉伸 + CLAHE + 轻去噪 + unsharp，可选边缘引导），仅作用分割/框提取，主图输出不变；配置 `preproc.*`（支持自适应触发与开关），run_summary 记录启用状态。分割后稳固化策略保持（平滑/面积保护、边缘衰减、凸包外扩、覆盖率/比例自适应）。
...
- 2025-12-18：回归运行 14/15 张（image_004 为 png 未跑），多数正常；022 覆盖率≈0.45，顶部做弱梯度削边，refine 覆盖≈0.999；016/019 因长宽比不符多轮重试后仍不过质检，按矩形回退并跳过透视；059 覆盖率不足回退矩形。需后续确认 022 粉框是否仍超绿框以及 image_004.png 补跑。
- 2025-12-18：补跑 image_004.png，15 张齐全。接手者请先目视 `outputs_sample15/image_022/02_debug_bbox.png`（确认顶部翻页削减、粉框位置）、复核 016/019 回退矩形是否可接受；如有调整需改完后更新本 DEVLOG，避免后续接手者看到过期的“下一步”提示。
